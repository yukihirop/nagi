...
### Check deploy/default/host/entry.ts exists

```bash
test -f deploy/default/host/entry.ts && echo "EXISTS" || echo "MISSING"
```
...
2. Common issues:
   ...
   - `deploy/default/host/entry.ts` missing → run `/deploy`
...
- Port already in use → another instance running. Kill it: `pkill -f "tsx deploy/default/host/entry.ts"`
...
After `pnpm build` or editing `deploy/default/host/entry.ts`:
```bash
launchctl kickstart -k gui/$(id -u)/com.nagi
```

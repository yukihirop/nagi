...
### Check entry.ts has Vercel registration

Read `entry.ts` and verify it contains `registerMcpPlugin("vercel"`. If not, add it.
...
Verify `entry.ts` contains the Vercel MCP plugin registration. If not, add this block after the orchestrator creation:
...
If entry.ts is outdated, compare with `entry.template.ts` and update accordingly.
...
The token must be in `.env` at the project root AND registered in `entry.ts` via `registerMcpPlugin` with `env: { VERCEL_API_TOKEN: ... }`.

### Agent doesn't see Vercel tools

1. Check `entry.ts` has `registerMcpPlugin("vercel", ...)`

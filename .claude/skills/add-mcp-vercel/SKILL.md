...
### Check apps/entry.ts has Vercel registration

Read `apps/entry.ts` and verify it contains `registerMcpPlugin("vercel"`. If not, add it.
...
Verify `apps/entry.ts` contains the Vercel MCP plugin registration. If not, add this block after the orchestrator creation:
...
If `apps/entry.ts` is outdated, compare with `apps/entry.template.ts` and update accordingly.
...
The token must be in `.env` at the project root AND registered in `apps/entry.ts` via `registerMcpPlugin` with `env: { VERCEL_API_TOKEN: ... }`.

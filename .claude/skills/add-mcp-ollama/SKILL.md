...
Verify `entry.ts` contains the Ollama MCP plugin registration. If not, add this block after the orchestrator creation:
...
1. Check `entry.ts` has `registerMcpPlugin("ollama", ...)`

...
Read `apps/entry.ts` and check which Slack import is used:
...
In `apps/entry.ts`, replace the Slack import:
...
To switch back to plain text, reverse the import in `apps/entry.ts`:

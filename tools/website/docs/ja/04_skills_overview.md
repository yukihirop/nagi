...
| [グループプロンプト](/ja/12_skills_group_prompt) | 3 | `create-group-prompt` / `update-group-prompt` / `configure-skills` | エージェント人格・指示の作成・編集、スキル allowlist |
...
| `/configure-skills` | プロンプト | グループ単位のスキル allowlist（`skills.config.json`）編集 |
